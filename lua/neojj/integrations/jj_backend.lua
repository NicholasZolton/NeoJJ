...
function M.find_jj_workspace(path)
  ...
  while p and p ~= "" and p ~= "/" do
    if vim.fn.isdirectory(p .. "/" .. JJ_DIR) == 1 then return p end
...
function M.resolve_jj_repo_dir(workspace)
  ...
  if stat.type == "directory" then return vim.fn.resolve(repo_path) end
...
function M.jj_backing_git_dir(workspace)
  ...
  if target then
    ...
    if vim.fn.isdirectory(resolved) == 1 then return resolved end
  end

  local internal = store_dir .. "/" .. INTERNAL_GIT_NAME
  if vim.fn.isdirectory(internal) == 1 then return internal end
  return nil
end
